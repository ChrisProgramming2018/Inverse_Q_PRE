...
class Agent():
    ...
    def learn(self, memory_ex):
        ...
        states = self.encoder.create_vector(states) #.detach()
        self.state_action_frq(states, actions)
        states, next_states, actions , batch_idx, tree_idx = memory_ex.sample(self.qnetwork_local, self.encoder, self.writer, self.steps)
        states = states.type(torch.float32).div_(255)
        ...
        memory_ex.update_weights(batch_idx, tree_idx, self.qnetwork_local, self.encoder)
    ...
    def test_q_value(self, memory):
        ...
        same_sh = 0
        size = 100
        test_elements = np.random.choice(memory.idx, size, replace=False)
        all_diff = 0
        q_error = 0
        r_error = 0
        error = True
        ...
        memory.kl_threshold = average_q_kl * 0.1
        if average_q_kl < 1 and self.optimizer.param_groups[0]['lr'] > 5e-5:
